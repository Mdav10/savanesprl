...
DATABASE_URL = os.getenv("DATABASE_URL", "postgresql://mlw_attack_user:ShJf3c9NA4Jf1ADITLYh3fIlHc7akHXC@dpg-d8063p9j2pic73f1mm40-a/mlw_attack")
...
# Create FastAPI app
app = FastAPI(
    title="SavaneSPRL - Système Financier Sécurisé",
    description="Système de gestion financière avec RBAC",
    version="1.0.0"
)
...
def read_root():
    return {
        "status": "active",
        "message": "SavaneSPRL API is running",
        "version": "1.0.0",
        "endpoints": {
            "auth": "/api/auth/login, /api/auth/register",
            "dashboard": "/api/dashboard/dg, /api/dashboard/commercial",
...
async def logout(current_user = Depends(get_current_user), request: Request = None, db: Session = Depends(get_db)):
    log = AuditLog(
...
async def dashboard_dg(current_user = Depends(role_required([RoleEnum.DG, RoleEnum.DAF])), db: Session = Depends(get_db)):
    # Financial stats
    entrees = db.query(Transaction).filter(
        Transaction.type == "entrée", 
        Transaction.statut == TransactionStatus.APPROUVE
    ).all()
    sorties = db.query(Transaction).filter(
    ...
    # Pending approvals
    en_attente = db.query(Transaction).filter(
    ...
    return {
        ...
        "recent_activities": [
            {"action": log.action, "timestamp": log.timestamp.isoformat() if log.timestamp else None}
...
async def dashboard_commercial(current_user = Depends(role_required([RoleEnum.DIRECTEUR_COMMERCIAL])), db: Session = Depends(get_db)):
    ...
    montant_total_ventes = sum(v.quantite * v.prix_unitaire for v in ventes)
...
async def create_transaction(
    ...
    current_user = Depends(role_required([RoleEnum.COMPTABLE])),
...
async def get_pending_transactions(
    current_user = Depends(role_required([RoleEnum.DG, RoleEnum.DAF])),
    db: Session = Depends(get_db)
):
...
async def approve_transaction(
    ...
    log = AuditLog(
        user_id=current_user.get("user_id"),
        action="APPROVAL",
        details=f"{message} - Transaction #{transaction.id}",
        ip_address="system"
    )
    db.add(log)
    db.commit()
...
async def get_transactions_history(
    current_user = Depends(role_required([RoleEnum.DG, RoleEnum.DAF, RoleEnum.COMPTABLE])),
    db: Session = Depends(get_db),
    limit: int = 100
):
...
async def create_vente(
    vente: VenteCreate,
    current_user = Depends(role_required([RoleEnum.AGENT_MARKETING])),
    db: Session = Depends(get_db)
):
    user_id = current_user.get("user_id")
    
    # Calculate total amount (hidden from agent response)
    ...
    if stock:
        ...
        stock = Stock(
            produit=vente.produit,
            quantite_sortie=vente.quantite,
            quantite_disponible=-vente.quantite
        )
        db.add(stock)
    ...
    return {
        ...
        "quantite": vente.quantite,
        "prix_unitaire": vente.prix_unitaire
        # Note: montant_total is NOT returned to agent
    }

@app.get("/api/ventes/my")
async def get_my_ventes(
    current_user = Depends(role_required([RoleEnum.AGENT_MARKETING])),
    db: Session = Depends(get_db)
):
    user_id = current_user.get("user_id")
...
async def get_all_ventes_commercial(
    current_user = Depends(role_required([RoleEnum.DIRECTEUR_COMMERCIAL])),
    db: Session = Depends(get_db)
):
...
async def create_user(
    user: UserCreate,
    current_user = Depends(role_required([RoleEnum.DG])),
    db: Session = Depends(get_db)
):
    # Check if user exists
...
async def add_stock(
    produit: str,
    quantite: int,
    current_user = Depends(role_required([RoleEnum.COMPTABLE, RoleEnum.DG])),
    db: Session = Depends(get_db)
):
...
async def get_all_stocks(
    current_user = Depends(role_required([RoleEnum.DIRECTEUR_COMMERCIAL, RoleEnum.DG, RoleEnum.DAF])),
    db: Session = Depends(get_db)
):
    stocks = db.query(Stock).all()
    return [
        {
            ...
            "quantite_sortie": s.quantite_sortie,
